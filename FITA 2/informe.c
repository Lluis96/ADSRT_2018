...
// Defineix punter a una estructura tm
	...
	char sqlTmax[80],sqlTmin[80],sqlTavg[80],sqlavgtimefan[80],sqlcounttimefan[80],sqlalarmestable[80];
	...
	char dataalarma[30]; //es fa servir callback
	char ton_fan[30]; //es fa servir callback

	char *De="1393272@campus.euss.org";
	char *To="1393272@campus.euss.org";
    
    // variables on guardar les consultes sql
	char Tmax[80],Tmin[80],Tavg[80],startdata[80], finishdata[80],tempsontotal[80],tempsavgtotal[80],numcopsfan[80]; 
...
void fecha_informe (){
   ...
   // printf ("sqlTmin:\t%s\n",sqlTavg);
    ...
    sprintf(sqlavgtimefan,"SELECT AVG (VENT) FROM TEMPERATURA WHERE DATA BETWEEN  '%s' AND '%s';",fecha_daybefore_dos,fecha_day);
   // printf ("sqlavgtimefan:\t%s\n",sqlavgtimefan);
   
    sprintf(sqlcounttimefan,"SELECT COUNT (VENT) FROM TEMPERATURA WHERE DATA BETWEEN '%s' AND '%s' AND VENT > 0;",fecha_daybefore_dos,fecha_day);
